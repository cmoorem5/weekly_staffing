...
Helpers used by **both** the weekly and quarterly PDF builders live in `staffing_tool/report_style.py` — `exception_table`, `base_coverage_table`, `trend_fig`, `exception_bar_fig`, `leave_rows`, `leave_top2`, `pct`, `short_label`, and the `data_table_style()` command prelude every data table opens with. The two builders keep only thin wrappers passing their own column ratios / figure height / legend label. These were duplicated copy-paste before; `tests/test_report_shared_helpers.py` pins the rendered result (cell values, resolved cell styling, column widths, chart series) so a change to one report can't silently alter the other. HTML tables go through `report_html.data_table` — don't re-implement it locally.
