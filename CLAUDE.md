...
- **One day editor serves both kinds.** `views/schedulers._day_editor` + `templates/crew_hub/schedule_day.html` are driven by `SCHEDULER_KINDS`; the form posts `action=add` (load people onto the day) or `action=save` (set each row's slot/work type/hours/note, or tick Remove), keyed by assignment pk. Adding a duty officer defaults to their rostered role; Comm staff start unassigned.
